...
function SwarmSpace(swarmType, utils) {
    ...
    function SwarmDescription(swarmTypeName, description){
        ...
        function createMembers(description) {
            var members = {};
            for(var prop in description){
                if(prop !== 'public' && prop !== 'local' && prop !== 'secure'){
                    members[prop] = description[prop];
                }
            }
            return members;
        }

        var public = createVars(description.public);
        var local = createVars(description.local);
        var secure = createVars(description.secure);
        var phases = createMembers(description);
        ...
        this.initialise = function(serialisedValues){
            ...
            for(var v in public){
                result.public[v] = public[v].init();
            };

            for(var v in local){
                result.local[v] = local[v].init();
            };

            for(var v in secure) {
              result.secure[v] = secure[v].init();
            }
        ...
        this.initialiseFunctions = function(valueObject, thisObject){
            ...
            for(var v in phases){
                valueObject.phases[v] = createPhase(thisObject, phases[v], v);
            };
        ...
        this.get = function(target, property, receiver){
          ...
          if (public.hasOwnProperty(property)) {
          ...
          if (phases.hasOwnProperty(property)) {
            return target.phases[property]
          }

          if (target.secure.hasOwnProperty(property)) {
      ...
      this.set = function(target, property, value, receiver){
          ...
          if(target.utilityFunctions.hasOwnProperty(property) || target.phases.hasOwnProperty(property)) {
          ...
          } else if (public.hasOwnProperty(property)) {
        ...
        this.has = function(target, property) {
            if(target.public[property] || target.secure[property] || target.local[property]) {
                return true;
            }
            return false;
        };
        ...
        this.ownKeys = function(target) {
            return Reflect.ownKeys(target.public);
        };
